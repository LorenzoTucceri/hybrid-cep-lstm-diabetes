                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="d-flex align-items-end">
                                ...
                                <div class="mb-3">
                                    <h5 class="font-size-15 mb-1">{{ Str::ucfirst(Auth::user()->name)." ".Str::ucfirst(Auth::user()->surname)}}</h5>
                                    <p class="text-muted mb-0 font-size-12 badge badge-soft-primary">{{ Str::ucfirst(Auth::user()->role->name) }}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="pt-3 border-top">
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <h6 class="font-size-14">{{ Str::ucfirst(Auth::user()->email)}}</h6>
                                    </div>
                                </div>
                                <div class="mt-3 d-grid">
                                    <a href="{{Route('userProfile')}}" class="btn btn-primary waves-effect waves-light btn-sm">
                                ...
                                <div class="card-body">
                                    <div class="d-flex">
                                        <div class="flex-grow-1">
                                            ...
                                            <h4 class="mb-0">
                                                {{ \App\Models\File::whereIn('patient_id', \App\Models\Patient::where('doctor_id', Auth::user()->id)->pluck('id'))->count() }}
                                            </h4>
                                        </div>
                                        <div class="flex-shrink-0 align-self-center">
                                            <div class="mini-stat-icon avatar-sm rounded-circle bg-soft-warning text-warning">
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <span class="text-truncate" style="max-width: 200px;" title="{{ $file->csv_file_path }}">{{ basename($file->csv_file_path) }}</span>
                                            </div>
                                        </td>
                                        <td>{{ \Carbon\Carbon::parse($file->start_time)->format('d M Y, H:i') }}</td>
                                        <td>{{ \Carbon\Carbon::parse($file->end_time)->format('d M Y, H:i') }}</td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center gap-2">
                                                <a href="{{ route('viewCsv', ['csvId' => $file->id, 'patientId' => auth()->user()->patient_id]) }}" target="_blank" class="btn btn-sm btn-soft-primary" data-bs-toggle="tooltip" title="Analyze">
                                                    <i class="mdi mdi-eye-outline font-size-14"></i>
                                                </a>
                                                <a href="javascript:void(0);" onclick="openFeedbackModal({{ $file->id }}, {{auth()->user()->patient->doctor_id}})" data-bs-toggle="modal" data-bs-target="#feedbackModal" class="btn btn-sm btn-soft-info" data-bs-toggle="tooltip" title="Feedback">
                                                    <i class="mdi mdi-comment-eye-outline font-size-14"></i>
                                                </a>
                                                <a href="javascript:void(0);" onclick="deleteCsv({{ $file->id }})" data-bs-toggle="modal" data-bs-target="#csvDelete" class="btn btn-sm btn-soft-danger" data-bs-toggle="tooltip" title="Delete">
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <h5 class="font-size-14 mb-0">{{$patient->name}} {{$patient->surname}}</h5>
                                                    </div>
                                                </td>
                                                <td>{{$patient->email}}</td>
                                                <td class="text-center">
                                                    <a href="{{ route('showCsvPatient', $patient->id) }}" class="btn btn-sm btn-soft-primary" data-bs-toggle="tooltip" title="View Files">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="feedbackText" class="form-label">
                                <strong>Doctor:</strong> {{ auth()->user()->patient->doctor->name." ".auth()->user()->patient->doctor->surname }}
                            </label>
