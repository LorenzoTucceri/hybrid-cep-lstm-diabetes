            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                        <div>
                            <h3 class="mb-1">Patient: {{ $patient->name.' '.$patient->surname }}</h3>
                            <p class="text-muted mb-0">Manage CSV files and AI Monitoring</p>
                        </div>

                        <div>
                            @if($patient->has_trained_model)
                                    ...
                                    <form action="{{ route('uploadCsv') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        @if(auth()->user()->role->name == "Patient")
                                            <input type="hidden" name="doctor"
                                                   value="{{ auth()->user()->patient ? auth()->user()->patient->doctor_id : '' }}">
                                        @else
                                        ...
                                        <div class="row align-items-end">
                                            <div class="col-md-8">
                                                ...
                                                <input id="csv" name="csv[]" type="file"
                                                       class="form-control @error('csv') is-invalid @enderror" multiple
                                                       required>
                                                @error('csv')
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <div class="text-truncate" style="max-width: 180px;" data-bs-toggle="tooltip"
                                             title="{{ basename($file->csv_file_path) }}">
                                            <span
                                                class="fw-medium text-dark">{{ basename($file->csv_file_path) }}</span>
                                        </div>

                                    </td>
                                    <td>{{ \Carbon\Carbon::parse($file->start_time)->format('Y-m-d H:i') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($file->end_time)->format('Y-m-d H:i') }}</td>
                                    <td data-gmi="{{ $file->gmi }}">
                                        <div class="d-flex align-items-center">
                                            <span class="fw-bold me-2">{{ $file->gmi }}%</span>
                                            @if($file->gmi < 7)
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            @if($targetId != 0)
                                                <a href="#feedbackModal"
                                                   onclick="openFeedbackModal({{ $file->id }}, {{ $isDoc ? 'true' : 'false' }}, {{ $targetId }})"
    ...
    <div class="modal fade" id="forecastingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg">
                ...
                <form action="{{ route('monitor.view') }}" method="POST" target="_blank">
                    @csrf
                    <div class="modal-body p-4">
                        <div class="text-center mb-4">
                            <p class="text-muted">Enter Dexcom credentials for <strong>{{ $patient->name }}</strong>.
                            </p>
                        </div>
                        <input type="hidden" name="patient_id" value="{{ $patient->id }}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-body px-4 py-5 text-center">
                    ...
                    <form action="{{ route('deleteCsv') }}" method="POST">@csrf<input type="hidden" name="csv_id"
                                                                                      id="boxDelete">
    ...
    @if(!$patient->has_trained_model)
        <script>
            ...
            let modelInterval = setInterval(function() {
                fetch("/api/check-model-status/{{ $patient->id }}")
